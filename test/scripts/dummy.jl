...
# Prot_system = "NS_CB"
Prot_system_coll = ["NS_CB", "FS_HDCCB"]
...
for proti = 1:2
    ...
    # year = ["NAT_2030_Generation","NAT_2040_Generation", "NAT_2050_Generation"]
    # year_num = ["2030", "2040", "2050"]    # year = ["NAT_2045_Generation"]
    ...
    st_nw = 0;
    end_nw = 0;
    function mp_contignecy(data_ip, Total_sample, base_weight_ip, weights)
        Cont_list = Any[]
    ...
    data_ps, cost = mp_datainputs(deepcopy(data_mp), Total_sample)
    base_weight_ip = Array{Float64}(undef, 1, no_nw)
    @save "costt.jld2" cost
    @load "cluster.jld2"
    data_cont, Cont_list, base_list, base_weight = mp_contignecy(deepcopy(data_ps), Total_sample*total_yr, base_weight_ip, weights)
        ...
        "Permanentloss" => false, "Cont_list" => Cont_list,"base_list" => base_list,"Total_sample" =>Total_sample, "curtailed_gen" => curtailed_gen, "max_curt" => max_curt, "syncarea" => syncarea, "weights" => base_weight)
        ...
        "Permanentloss" => true, "Cont_list" => Cont_list,"base_list" => base_list,"Total_sample" =>Total_sample, "curtailed_gen" => curtailed_gen, "max_curt" => max_curt, "syncarea" => syncarea, "weights" => base_weight)
        end
    ...
    if occursin("4bus", file)
        filepath = string("C:\\Users\\djaykuma\\OneDrive - Energyville\\Freq_TNEP_paper\\MATLAB\\plots\\TNEP\\4bus\\",Prot_system,"po_cl.xlsx")
    elseif occursin("6bus", file)
        filepath = string("C:\\Users\\djaykuma\\OneDrive - Energyville\\Freq_TNEP_paper\\MATLAB\\plots\\TNEP\\6bus\\",Prot_system,"_cl.xlsx")
    end
