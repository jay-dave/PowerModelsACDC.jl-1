...
function add_ref_dcgrid!(ref::Dict{Symbol,<:Any}, data::Dict{String,<:Any})
    for (n, nw_ref) in ref[:nw]
        ...
        # display(bus_convs_dirct)
            ...
            # display("bus_arcs_conv")
            # display(bus_arcs_conv)


            bus_arcs_sync = Dict([(reserve["syncarea"], []) for (i,reserve) in nw_ref[:reserves]])
            for (l,i,j) in nw_ref[:arcs_sync_conv]
                push!(bus_arcs_sync[j], i)
            end
            nw_ref[:bus_arcs_sync] = bus_arcs_sync
            # display(bus_arcs_sync)
            ...
            for (i,conv) in nw_ref[:convdc]
                 # display(conv)
                 push!(bus_syc_arc[conv["busac_i"]],conv["syncarea"])
            end
            # display(bus_syc_arc)
        end
    # end of  of OPF OPF OPF OPF
