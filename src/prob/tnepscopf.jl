...
function post_mp_tnepscopf(pm::_PM.AbstractPowerModel)
    ...
    for (n, networks) in pm.ref[:nw]
        ...
        #slack for freqeuncy constraints
        # variable_power_dc_KCL_slack(pm; nw = n)
        # variable_power_ac_KCL_slack(pm; nw = n)
    ...
    for (n, networks) in pm.ref[:nw]
     ...
     # constraint_slack_on_off(pm)
     constraint_frequency_stab_TNEP(pm)
end
