export run_acdcscopf_sensitivity

""
function run_acdcscopf_sensitivity(file::String, model_type::Type, solver; kwargs...)
    data = _PM.parse_file(file)
    PowerModelsACDC.process_additional_data!(data)
    return run_acdcscopf_sensitivity(data, model_type, solver; ref_extensions = [add_ref_dcgrid!], kwargs...)
end

""
function run_acdcscopf_sensitivity(data::Dict{String,Any}, model_type::Type, solver; kwargs...)
    return _PM.run_model(data, model_type, solver, post_acdcscopf_sensitivity; ref_extensions = [add_ref_dcgrid!], kwargs...)
    # return PowerModels.optimize_model!(pm, solver; solution_builder = get_solution_acdc)
end

""
function post_acdcscopf_sensitivity(pm::_PM.AbstractPowerModel)
    ...
    # variable_frequency_stab_OPF(pm; nw = n)
    ...
    display("curtailment_prof")

    # for (n, networks) in pm.ref[:nw]
    objective_min_cost_OPF_sensitivity(pm)
    # end
    ...
    for i in _PM.ids(pm, :convdc)
     ...
     constraint_frequency_stab_OPF_sensitivity(pm)
end
